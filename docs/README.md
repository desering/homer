...
Hosted using GitHub Pages at https://start.desering.org/.

# Contribution
Edit `config.yml` and `customizations.css`.

## Most accessible, but silly way
...
To test locally you can run the build action with:
...
As suggested in [Homer docs subpage](https://github.com/bastienwirtz/homer/blob/main/public/assets/icons/README.md) let's use [RealFaviconGenerator](https://realfavicongenerator.net/) for generating a favicon bundle:

The settings are already in `favicon-settings.json` and the deployment step generates a favicon bundle based on an icon from the `media` repository (see GitHub Actions file).
